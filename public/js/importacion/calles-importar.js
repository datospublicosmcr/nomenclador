/**
 * Nomenclador — Importación masiva de Calles desde CSV
 *
 * Flujo de 3 pasos:
...
const ImportarCalles = {
    ...
    abrirModal() {
        this.resetearEstado();
        document.getElementById('modalImportarCalles').classList.add('show');
        this.mostrarPaso(1);
    },

    cerrarModal() {
        document.getElementById('modalImportarCalles').classList.remove('show');
        this.resetearEstado();
    },
    ...
    resetearEstado() {
        ...
        if (fi) fi.value = '';
        const prev = document.getElementById('ic-archivo-preview');
        if (prev) prev.innerHTML = '';
        const ck = document.getElementById('ic-check-confirm');
        if (ck) ck.checked = false;
        const btn = document.getElementById('ic-btn-procesar');
    ...
    mostrarPaso(paso) {
        document.querySelectorAll('#modalImportarCalles .importar-paso').forEach((p, i) => {
            p.classList.toggle('active', i + 1 === paso);
        });
        document.querySelectorAll('#modalImportarCalles .paso-indicador').forEach((ind, i) => {
            ind.classList.remove('active', 'completado');
            if (i + 1 === paso) ind.classList.add('active');
            else if (i + 1 < paso) ind.classList.add('completado');
        });
        document.querySelectorAll('#modalImportarCalles .paso-separador').forEach((sep, i) => {
        ...
        show('ic-btn-procesar', paso === 1);
        show('ic-btn-volver1',  paso === 2);
        show('ic-btn-siguiente',paso === 2);
        show('ic-btn-volver2',  paso === 3);
        show('ic-btn-aplicar',  paso === 3);

        if (paso === 3) {
            const btn = document.getElementById('ic-btn-aplicar');
    ...
    async procesarCSV() {
        const input = document.getElementById('ic-file');
        const file  = input.files[0];
        if (!file) return;
        ...
        try {
            ...
            const resultado = await api('/calles/importar/preview', {
                method: 'POST',
    ...
    _parsearCSV(texto) {
        ...
        const alias = {
            orden_carga:       ['orden_carga', 'orden', 'oc', 'id'],
        ...
        if (idx.nombre_calle === undefined) throw new Error('No se encontró la columna "nombre_calle" en el CSV');

        return lineas.slice(1).map(l => {
            const vals = this._parsearLinea(l);
            return {
                orden_carga:       idx.orden_carga       !== undefined ? vals[idx.orden_carga]       || '' : '',
                nombre_calle:      vals[idx.nombre_calle]      || '',
                observacion_calle: idx.observacion_calle !== undefined ? vals[idx.observacion_calle] || '' : ''
            };
        }).filter(r => r.nombre_calle.trim());
    },
    ...
    renderizarEstadisticas() {
        const el = document.getElementById('ic-stats');
        if (!el) return;
    ...
    renderizarTabs() {
        const el = document.getElementById('ic-tabs');
        if (!el) return;
        const t = this.tabActual;
        let h = '';
        h += `<button class="tab-btn tab-nuevos ${t==='nuevos'?'active':''}" onclick="ImportarCalles.cambiarTab('nuevos')">Nuevos (${this.nuevos.length})</button>`;
        h += `<button class="tab-btn tab-actualizar ${t==='actualizar'?'active':''}" onclick="ImportarCalles.cambiarTab('actualizar')">Actualizar (${this.actualizar.length})</button>`;
        h += `<button class="tab-btn ${t==='sinCambios'?'active':''}" onclick="ImportarCalles.cambiarTab('sinCambios')">Sin cambios (${this.sinCambios.length})</button>`;
        if (this.eliminados.length > 0)
            h += `<button class="tab-btn tab-eliminados ${t==='eliminados'?'active':''}" onclick="ImportarCalles.cambiarTab('eliminados')">Eliminados (${this.eliminados.length})</button>`;
        if (this.duplicadosCSV.length > 0)
            h += `<button class="tab-btn ${t==='duplicadosCSV'?'active':''}" onclick="ImportarCalles.cambiarTab('duplicadosCSV')">Dup. CSV (${this.duplicadosCSV.length})</button>`;
        if (this.errores.length > 0)
            h += `<button class="tab-btn tab-errores ${t==='errores'?'active':''}" onclick="ImportarCalles.cambiarTab('errores')">Errores (${this.errores.length})</button>`;
        el.innerHTML = h;
    },
    ...
    renderizarContenidoTab() {
        const el = document.getElementById('ic-tabla');
        if (!el) return;
    ...
    _tablaNuevos(el) {
        if (!this.nuevos.length) { el.innerHTML = '<div class="tabla-vacia">No hay calles nuevas para importar</div>'; return; }
        const todos = this.nuevos.length === this.seleccionNuevos.size;
        let h = `<div class="tabla-acciones">
            <label class="checkbox-todos"><input type="checkbox" ${todos?'checked':''} onchange="ImportarCalles._toggleTodos('nuevos',this.checked)"> Seleccionar todos</label>
            ...
            <thead><tr><th class="col-check"></th><th>OC</th><th>Nombre</th><th>Observaciones</th></tr></thead><tbody>`;
        this.nuevos.forEach((item, i) => {
            const s = this.seleccionNuevos.has(i);
            h += `<tr class="${s?'seleccionado':''}">
                <td class="col-check"><input type="checkbox" ${s?'checked':''} onchange="ImportarCalles._toggleSel('nuevos',${i},this.checked)"></td>
                <td>${item.datosCSV.orden_carga ?? '<span class="valor-vacio">—</span>'}</td>
                <td>${escapeHtml(item.datosCSV.nombre_calle)}</td>
                <td>${item.datosCSV.observacion_calle ? escapeHtml(item.datosCSV.observacion_calle) : '<span class="valor-vacio">—</span>'}</td>
            </tr>`;
        });
    ...
    _tablaActualizar(el) {
        if (!this.actualizar.length) { el.innerHTML = '<div class="tabla-vacia">No hay calles para actualizar</div>'; return; }
        const todos = this.actualizar.length === this.seleccionActualizar.size;
        let h = `<div class="tabla-acciones">
            <label class="checkbox-todos"><input type="checkbox" ${todos?'checked':''} onchange="ImportarCalles._toggleTodos('actualizar',this.checked)"> Seleccionar todos</label>
            <span class="seleccionados">${this.seleccionActualizar.size} de ${this.actualizar.length}</span></div>
            <div class="tabla-scroll"><table class="tabla-importar">
            <thead><tr><th class="col-check"></th><th>OC</th><th>Campo</th><th>Valor actual (BD)</th><th>Valor nuevo (CSV)</th></tr></thead><tbody>`;
        this.actualizar.forEach((item, i) => {
            ...
            item.cambios.forEach((cambio, j) => {
                ...
                h += `<tr class="${s?'seleccionado':''} ${clsInicio}">
                    ${j===0 ? `<td class="col-check" rowspan="${item.cambios.length}"><input type="checkbox" ${s?'checked':''} onchange="ImportarCalles._toggleSel('actualizar',${i},this.checked)"></td><td rowspan="${item.cambios.length}"><strong>${item.datosBD.orden_carga}</strong></td>` : ''}
                    <td>${cambio.label}</td>
    ...
    _tablaSinCambios(el) {
        if (!this.sinCambios.length) { el.innerHTML = '<div class="tabla-vacia">No hay calles sin cambios</div>'; return; }
        let h = '<div class="tabla-scroll"><table class="tabla-importar"><thead><tr><th>OC</th><th>Nombre</th></tr></thead><tbody>';
        this.sinCambios.forEach(item => {
            h += `<tr><td>${item.orden_carga??'—'}</td><td>${escapeHtml(item.nombre_calle)}</td></tr>`;
        });
        el.innerHTML = h + '</tbody></table></div>';
    },

    _tablaEliminados(el) {
        if (!this.eliminados.length) { el.innerHTML = '<div class="tabla-vacia">No hay calles eliminadas encontradas</div>'; return; }
        ...
        let h = `<div style="margin-bottom:12px;padding:10px 14px;background:#fef3c7;border-radius:8px;color:#92400e;font-size:0.83rem">
            <strong>Estas calles fueron eliminadas anteriormente.</strong> Si las seleccionás, serán reactivadas con los datos del CSV.</div>
            <div class="tabla-acciones">
            <label class="checkbox-todos"><input type="checkbox" ${todos?'checked':''} onchange="ImportarCalles._toggleTodos('reactivar',this.checked)"> Seleccionar todos</label>
            <span class="seleccionados">${this.seleccionReactivar.size} de ${this.eliminados.length}</span></div>
            <div class="tabla-scroll"><table class="tabla-importar">
            <thead><tr><th class="col-check"></th><th>OC</th><th>Nombre en BD</th><th>Nombre en CSV</th></tr></thead><tbody>`;
        this.eliminados.forEach((item, i) => {
            const s = this.seleccionReactivar.has(i);
            h += `<tr class="${s?'seleccionado':''}">
                <td class="col-check"><input type="checkbox" ${s?'checked':''} onchange="ImportarCalles._toggleSel('reactivar',${i},this.checked)"></td>
                <td>${item.datosBD.orden_carga}</td>
                <td><span class="valor-anterior">${escapeHtml(item.datosBD.nombre_calle)}</span></td>
                <td><span class="valor-nuevo">${escapeHtml(item.datosCSV.nombre_calle)}</span></td>
            </tr>`;
        });
        el.innerHTML = h + '</tbody></table></div>';
    },
    ...
    _tablaDuplicados(el) {
        ...
        let h = `<div style="margin-bottom:12px;padding:10px 14px;background:#fee2e2;border-radius:8px;color:#b91c1c;font-size:0.83rem">
            <strong>Estos OC aparecen más de una vez en el CSV.</strong> Corregí el archivo y volvé a importar.</div>
            <div class="tabla-scroll"><table class="tabla-importar">
            <thead><tr><th>OC</th><th>Fila</th><th>Nombre</th></tr></thead><tbody>`;
        this.duplicadosCSV.forEach(dup => {
            dup.registros.forEach((reg, j) => {
                h += `<tr class="fila-error">
                    ${j===0?`<td rowspan="${dup.registros.length}"><strong>${dup.orden_carga}</strong></td>`:''}
                    <td>${reg.fila}</td><td>${escapeHtml(reg.nombre_calle)}</td>
                </tr>`;
            });
        });
        el.innerHTML = h + '</tbody></table></div>';
    ...
    _tablaErrores(el) {
        ...
        let h = '<div class="tabla-scroll"><table class="tabla-importar"><thead><tr><th>Fila</th><th>OC</th><th>Nombre</th><th>Error</th></tr></thead><tbody>';
        this.errores.forEach(err => {
            h += `<tr class="fila-error">
                <td>${err.fila}</td>
                <td>${err.registro?.orden_carga||'—'}</td>
                <td>${err.registro?.nombre_calle ? escapeHtml(err.registro.nombre_calle) : '—'}</td>
    ...
    _actualizarInfo() {
        ...
        const el = document.getElementById('ic-info-sel');
    ...
    renderizarPaso3() {
        const el = document.getElementById('ic-paso3-contenido');
        if (!el) return;
        ...
        let h = '<div class="confirmacion-scroll" id="ic-scroll-confirm">';

        if (nSel.length) {
            h += `<div class="confirmacion-seccion"><div class="confirmacion-titulo">➕ Se insertarán ${nSel.length} calles nuevas</div><div class="confirmacion-lista">`;
            nSel.forEach(item => {
                h += `<div class="confirmacion-item" style="border-left-color:#1d4ed8">
                    <span class="item-interno">${item.datosCSV.orden_carga != null ? 'OC ' + item.datosCSV.orden_carga : 'Sin OC'}</span>
                    <span class="item-nombre">${escapeHtml(item.datosCSV.nombre_calle)}</span></div>`;
            });
            h += '</div></div>';
        }

        if (aSel.length) {
            h += `<div class="confirmacion-seccion"><div class="confirmacion-titulo">✏️ Se actualizarán ${aSel.length} calles</div><div class="confirmacion-lista">`;
            aSel.forEach(item => {
                h += `<div class="confirmacion-item" style="border-left-color:#f59e0b">
                    <span class="item-interno">OC ${item.datosBD.orden_carga}</span>
                    <span class="item-nombre">${escapeHtml(item.datosBD.nombre_calle)}</span>
        ...
        if (rSel.length) {
            h += `<div class="confirmacion-seccion"><div class="confirmacion-titulo">🔄 Se reactivarán ${rSel.length} calles eliminadas</div><div class="confirmacion-lista">`;
            rSel.forEach(item => {
                h += `<div class="confirmacion-item" style="border-left-color:#be185d">
                    <span class="item-interno">OC ${item.datosCSV.orden_carga}</span>
                    <span class="item-nombre">${escapeHtml(item.datosCSV.nombre_calle)}</span></div>`;
            });
            h += '</div></div>';
        }
        ...
        h += `</div>
            <div class="confirmacion-footer">
                <label class="checkbox-confirmacion disabled" id="ic-label-confirm">
                    <input type="checkbox" id="ic-check-confirm" disabled onchange="ImportarCalles._toggleConfirm(this.checked)">
    ...
    _initScrollDetector() {
        const scroll = document.getElementById('ic-scroll-confirm');
        const ck     = document.getElementById('ic-check-confirm');
        const lbl    = document.getElementById('ic-label-confirm');
    ...
    async aplicarImportacion() {
        ...
        try {
            const resultado = await api('/calles/importar/aplicar', {
                method: 'POST',
    ...
    _mostrarResultado(resultado) {
        ...
        let stats = `<div class="resultado-stat insertados"><div class="stat-numero">${r.total_insertados}</div><div class="stat-texto">Insertadas</div></div>
                     <div class="resultado-stat actualizados"><div class="stat-numero">${r.total_actualizados}</div><div class="stat-texto">Actualizadas</div></div>`;
        if (r.total_reactivados > 0) stats += `<div class="resultado-stat revisar"><div class="stat-numero">${r.total_reactivados}</div><div class="stat-texto">Reactivadas</div></div>`;
        ...
        el.innerHTML = `
            <div class="resultado-importacion">
                ...
                <div class="resultado-acciones">
                    <button class="btn btn-secondary" onclick="ImportarCalles._descargarReporte()">Descargar reporte XLSX</button>
                    <button class="btn btn-primary" onclick="ImportarCalles.finalizarImportacion()">Cerrar</button>
                </div>
            </div>`;

        const btnV2 = document.getElementById('ic-btn-volver2');
        const btnAp = document.getElementById('ic-btn-aplicar');
    ...
    async finalizarImportacion() {
        this.cerrarModal();
        if (typeof cargarCalles === 'function') await cargarCalles();
    },
    ...
    _descargarReporte() {
        ...
        const wsIns = XLSX.utils.json_to_sheet(
            res.insertados?.length
                ? res.insertados.map(i => ({ 'OC': i.orden_carga ?? '', 'Nombre': i.nombre_calle }))
                : [{ 'OC': '', 'Nombre': 'No se insertaron calles' }]
        );
        wsIns['!cols'] = [{ wch: 10 }, { wch: 45 }];
        XLSX.utils.book_append_sheet(wb, wsIns, 'Insertadas');

        let dataAct = [];
        if (res.actualizados?.length) {
            res.actualizados.forEach(a => a.cambios.forEach((c, i) => {
                dataAct.push({ 'OC': i===0?a.orden_carga:'', 'Nombre': i===0?a.nombre_calle:'',
                    'Campo': c.campo, 'Valor anterior': c.valor_anterior||'', 'Valor nuevo': c.valor_nuevo||'' });
            }));
        } else dataAct = [{ 'OC': '', 'Nombre': 'No se actualizaron calles', 'Campo': '', 'Valor anterior': '', 'Valor nuevo': '' }];
        const wsAct = XLSX.utils.json_to_sheet(dataAct);
        wsAct['!cols'] = [{ wch: 10 }, { wch: 35 }, { wch: 15 }, { wch: 30 }, { wch: 30 }];
        XLSX.utils.book_append_sheet(wb, wsAct, 'Actualizadas');

        if (res.reactivados?.length) {
            const wsR = XLSX.utils.json_to_sheet(res.reactivados.map(r => ({ 'OC': r.orden_carga??'', 'Nombre': r.nombre_calle })));
            wsR['!cols'] = [{ wch: 10 }, { wch: 45 }];
            XLSX.utils.book_append_sheet(wb, wsR, 'Reactivadas');
        }

        if (res.errores?.length) {
            const wsE = XLSX.utils.json_to_sheet(res.errores.map(e => ({ 'Tipo': e.tipo, 'OC': e.orden_carga||'', 'Nombre': e.nombre||'', 'Error': e.error })));
            wsE['!cols'] = [{ wch: 12 }, { wch: 10 }, { wch: 35 }, { wch: 50 }];
            XLSX.utils.book_append_sheet(wb, wsE, 'Errores');
        }

        XLSX.writeFile(wb, `reporte_calles_${new Date().toISOString().slice(0,10)}.xlsx`);
